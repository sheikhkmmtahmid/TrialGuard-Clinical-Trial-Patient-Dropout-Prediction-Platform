...
from core.utils.data_pipeline import (
    ...
    FEATURE_COLUMNS, MODEL_FEATURE_COLUMNS, generate_synthetic_patients
)
...
from core.utils.shap_explainer import (
    build_shap_explainer, compute_shap_values, shap_values_to_json,
    compute_shap_stability_score, compute_shap_robustness_score,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        X_scaled = scaler.transform(df[MODEL_FEATURE_COLUMNS])
        self.stdout.write(self.style.SUCCESS(f'  ✓ Scaler fitted on {len(MODEL_FEATURE_COLUMNS)} features'))
        ...
        # X_with_hr is already scaled (via the fitted StandardScaler) with
        # hazard_ratio appended, the exact representation every real
        # prediction uses. Training must use this same representation, not
        # raw unscaled values, otherwise the model learns one thing and is
        # asked about another at prediction time.
        y = df['dropout_status'].values
        xgb_model, metrics = train_xgboost_model(X_with_hr, y, n_optuna_trials=n_trials)
        self.stdout.write(self.style.SUCCESS(
            f'  ✓ XGBoost trained — '
        ...
        # A modest background sample lets the explainer use interventional
        # perturbation, which handles correlated features properly instead
        # of splitting credit between them arbitrarily. Kept small (100
        # rows) since interventional mode is slower than the default.
        bg_rng = np.random.default_rng(42)
        bg_size = min(100, len(X_with_hr))
        background_X = X_with_hr[bg_rng.choice(len(X_with_hr), size=bg_size, replace=False)]
        explainer = build_shap_explainer(xgb_model, background_X=background_X)

        sample_size = min(500, len(X_with_hr))
        # Use SAME feature matrix as XGBoost (includes hazard_ratio)
        shap_sample = X_with_hr[:sample_size]
        ...
        # This is the metric that actually matters for trust: does the SAME
        # patient get a consistent explanation under small, realistic input
        # noise. See compute_shap_robustness_score's docstring for why this
        # is a different question from shap_stability_score above.
        robustness = compute_shap_robustness_score(explainer, X_with_hr)
        metrics['shap_robustness_score'] = robustness

        self.stdout.write(self.style.SUCCESS(
            f'  ✓ SHAP explainer built — cross-patient similarity: {stability:.4f}, '
            f'same-patient robustness: {robustness:.4f}'
        ))
        ...
        for patient in patients:
            ...
            try:
                ...
                X_row_df = last_row[MODEL_FEATURE_COLUMNS].copy()
                ...
                prob = _safe_float(xgb_model.predict_proba(X_full)[0][1], default=0.0)
                prob = min(max(prob, 0.0), 1.0)
                risk_tier = PredictionResult.get_risk_tier(prob)

                row_shap = explainer.shap_values(X_full)
                if isinstance(row_shap, list):
                    row_shap = row_shap[1]
                shap_json = shap_values_to_json(row_shap[0], MODEL_FEATURE_COLUMNS + ['hazard_ratio'])
                surv_info = predict_survival(hr_input)
